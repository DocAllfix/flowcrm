...
// Il controllo sul peso di avvio vive qui e non in uno script di package.json
// perche' la CI lancia `npx vite build` nudo: un controllo appeso a
// `npm run build` non girerebbe proprio dove deve fermare le regressioni.
import { controlloPesoAvvio } from './scripts/peso-avvio.mjs'

// https://vite.dev/config/
export default defineConfig({
  plugins: [react(), tailwindcss(), controlloPesoAvvio(__dirname)],
  resolve: {
    alias: {
